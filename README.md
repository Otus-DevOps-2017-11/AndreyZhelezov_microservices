...
Для настройки Ingress была использованан возможность, предоставляемая GKE, которая называется **Балансировка нагрузки HTTP**. Для её применения был создан манифест **ui-ingress.yml** который был настроен на простую балансировку нагрузки на сервисы ui с внешнего порта tcp:80. Было отмечено что Ingress направляет трафик с внешнего порта на сервис через сконфигурированный в сервисе _NodePort_.
...
## Подготовка.
На рабочем компьютере были установлены **kubectl** и **minikube**. Был развернут minikube-кластер kubernetes. Текущий конфиг(контекст) **kubectl** был переключен на minikube-кластер.
## Minikube-кластер.
...
Было добавлено новое окружение в minikube-кластер: **dev**. Сделано это при помощи применения манифеста **dev-namespaces.yml**, после применения которого в minikube-кластере в новом окружении было развернуто наше приложение ещё раз.
## GKE-кластер.
...
В GKE-кластере так же как и в minicube был запущен dashboard. Он был включен как компонент кластера под названием "Панель управления Kubernetes". И, хотя проблем с наличием учетной записи **kube-system:kubernetes-dashboard** в кластере не возникло, доступа у этой учетки к чтению состояния кластера не оказалось. Для того чтобы исправить эту ситуацию, была сделана привязка данной учетной записи к роли **cluster-admin** нашего кластера. После чего авторизация в RBAC прошла успешно и **dashboard** получил доступ к данным о состоянии кластера.
