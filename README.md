...
Для выполнения ДЗ в GCP создан docker-host **vm1** и окружение docker-client переключено на работу с ним. С помощью готового образа с Docker Hub был запущен контейнер с Prometeus.
## Работа с Prometeus.
Подключившись к веб-интерфейсу Prometeus, изучаем интерфейс. И останавливаем контейнер.
