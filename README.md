...
На платформе GXP создана ВМ(нода) **master-1**. На ней инициирован кластер docker swarm. Соданы ещё две ноды **worker-1** и **worker-2**. При помощи токена полученого на **master-1** ноды **worker-1** и **worker-2** добалены в кластер с ролью _worker_. 
