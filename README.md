...
### Фильтры.
В файл **docker/fluentd/fluent.conf** добавлен фильтр который для логов с полем **@log_name:**service.post распарсивает значение поля **log** на отдельные поля. Это позволило производить поиск по новым полям, например: _params.title:*kibana_ - что выводит логи в которых записано событие о создании поста с названием оканчивающимся на _kibana_.
